...
function ResultCard({ result, followedProfiles, followedOrganizations }: { result: MixedResult; followedProfiles: string[]; followedOrganizations: string[] }) {
  if (result.type === "person") {
    const p = result.item
    return <article className="rounded-card border border-brand-borderLight bg-white p-5 shadow-soft"><Link href={p.username ? `/u/${p.username}` : `/freelancers/${p.id}`}><p className="text-[10px] font-extrabold tracking-[.14em] text-brand-indigo">PROFESSIONAL</p><b className="mt-2 block text-brand-midnight">{p.full_name || "Professional"}</b><p className="text-caption text-brand-slate">{p.tagline || p.skills?.slice(0, 2).join(" · ")}</p></Link><FollowButton type="profile" id={p.id} initial={followedProfiles.includes(p.id)} /></article>
  }
  if (result.type === "organization") {
    const o = result.item
    const label = o.entity_type === "company" ? "COMPANY" : "ORGANIZATION"
    return <article className="rounded-card border border-brand-borderLight bg-white p-5 shadow-soft"><Link href={`/u/${o.username}`}><p className="text-[10px] font-extrabold tracking-[.14em] text-brand-indigo">{label}</p><b className="mt-2 block text-brand-midnight">{o.name}</b><p className="text-caption text-brand-slate">{o.tagline || o.industry || label}</p></Link><FollowButton type="organization" id={o.id} initial={followedOrganizations.includes(o.id)} /></article>
  }
  ...
  return <Link href={href} className="rounded-card border border-brand-borderLight bg-white p-5 shadow-soft"><p className="text-[10px] font-extrabold tracking-[.14em] text-brand-indigo">{label}</p><b className="mt-2 block text-brand-midnight">{item.title}</b></Link>
}
