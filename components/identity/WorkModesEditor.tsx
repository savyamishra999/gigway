...
export default function WorkModesEditor({ userId }: { userId: string }) {
  ...
  useEffect(() => { supabase.from("profile_intents").select("intent").eq("profile_id", userId).then(({ data }) => setModes((data ?? []).map(x => x.intent).filter((intent): intent is WorkMode => WORK_MODES.some(x => x.value === intent)))) }, [userId]) // eslint-disable-line react-hooks/exhaustive-deps
  const save = async () => { setSaving(true); setMessage(""); const { error: removeError } = await supabase.from("profile_intents").delete().eq("profile_id", userId); if (removeError) { setMessage(removeError.message); setSaving(false); return }; if (modes.length) { const { error } = await supabase.from("profile_intents").insert(modes.map(intent => ({ profile_id: userId, intent }))); if (error) { setMessage(error.message); setSaving(false); return } }; setMessage("Work Modes saved."); setSaving(false) }
