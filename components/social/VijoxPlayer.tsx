...
export default function VijoxPlayer({ src, duration }: { src: string; duration?: number | null }) {
  const audio = useRef<HTMLAudioElement>(null), id = useRef(`vijox-${crypto.randomUUID()}`);
  const [playing, setPlaying] = useState(false), [current, setCurrent] = useState(0);
  const total = duration || 27, label = (seconds: number) => `00:${Math.floor(seconds).toString().padStart(2, "0")}`;
  useEffect(() => { const pauseOther = (event: Event) => { if ((event as CustomEvent<string>).detail !== id.current) audio.current?.pause(); }; window.addEventListener(MEDIA_PLAY_EVENT, pauseOther); return () => window.removeEventListener(MEDIA_PLAY_EVENT, pauseOther); }, []);
  const toggle = async () => { const player = audio.current; if (!player) return; if (player.paused) { announceMediaPlay(id.current); await player.play(); setPlaying(true); } else { player.pause(); setPlaying(false); } };
