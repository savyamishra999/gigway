...
export default function CreatePostComposer({ profile, organizations }: Props) {
    const router = useRouter(), input = useRef<HTMLInputElement>(null), [body, setBody] = useState(""), [visibility, setVisibility] = useState("public"), [author, setAuthor] = useState("personal"), [files, setFiles] = useState<File[]>([]), [error, setError] = useState(""), [state, setState] = useState<"idle" | "uploading" | "publishing" | "posted">("idle");
