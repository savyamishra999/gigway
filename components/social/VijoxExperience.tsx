...
export type VijoxExperienceProps = {
  src: string;
...
export default function VijoxExperience({
  ...
  const startVisualizing = () => {
    ...
    if (!player || analyser.current) return;
    try {
      const nextContext = new AudioContext(),
        nextSource = nextContext.createMediaElementSource(player),
      ...
      const values = new Uint8Array(nextAnalyser.fftSize);
      const tick = () => {
        nextAnalyser.getByteTimeDomainData(values);
        let sum = 0;
        values.forEach((value) => {
          const normalized = (value - 128) / 128;
          sum += normalized * normalized;
        });
        setEnergy(Math.min(1, Math.sqrt(sum / values.length) * 5));
        frame.current = requestAnimationFrame(tick);
      };
      nextContext.resume().then(tick).catch(stopVisualizing);
    } catch {
      setEnergy(0);
    }
  };
  useEffect(() => {
    const pauseOther = (event: Event) => {
      if ((event as CustomEvent<string>).detail !== id.current)
        audio.current?.pause();
    };
    window.addEventListener(MEDIA_PLAY_EVENT, pauseOther);
    return () => {
      window.removeEventListener(MEDIA_PLAY_EVENT, pauseOther);
      teardown();
    };
  }, []);
  const toggle = async () => {
  ...
  const progress = Math.min(100, (current / Math.max(1, total)) * 100),
    degrees = Math.max(16, progress * 3.6),
    ring = {
      background: `conic-gradient(from -90deg,#ec4899 0deg,#8b5cf6 ${degrees}deg,#4f46e5 ${degrees + 34}deg,#06b6d4 ${degrees + 68}deg,#e8eaf0 ${degrees + 68}deg 360deg)`,
      transform: `scale(${1 + energy * 0.055})`,
    };
  return (
    <div
      className={`rounded-2xl border border-violet-200 bg-gradient-to-br from-pink-50 via-white to-cyan-50 ${compact ? "p-3" : "p-4"}`}
    >
      <audio
        ref={audio}
        src={src}
      ...
      />
      {imageUrl && (
        <div className="relative -mx-3 -mt-3 mb-3 overflow-hidden rounded-t-xl bg-brand-ivory sm:-mx-4 sm:-mt-4">
          <img src={imageUrl} alt={imageAlt} className="h-44 w-full object-cover sm:h-56" />
          <div aria-hidden className="absolute inset-x-0 bottom-0 h-20 bg-gradient-to-t from-white/95 to-transparent" />
        </div>
      )}
      <div className="flex items-center gap-3">
        <div
          className={`grid ${compact ? "h-16 w-16" : "h-20 w-20"} shrink-0 place-items-center rounded-full p-1.5 shadow-[0_6px_20px_rgba(124,58,237,.14)] motion-reduce:transform-none`}
          style={ring}
        >
