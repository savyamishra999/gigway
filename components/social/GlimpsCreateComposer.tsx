...
import { Hash, Loader2, Upload, Video, X } from "lucide-react";
...
export default function GlimpsCreateComposer({ profile, organizations }: Props) {
  ...
  const isRecorded = file?.name.startsWith("glimps-recording-");
  ...
  return <section className="rounded-3xl border border-brand-borderLight bg-white p-4 pb-[calc(6.5rem+env(safe-area-inset-bottom))] text-brand-midnight shadow-elevated sm:p-7 sm:pb-7">
    ...
    {!file ? <div className={capturing ? "mt-5" : "mt-5 grid grid-cols-2 gap-3"}><GlimpsCameraCapture onVideo={(next) => void selectFile(next)} onCaptureStateChange={setCapturing} />{!capturing && <button type="button" onClick={() => input.current?.click()} onDragOver={(event) => event.preventDefault()} onDrop={onDrop} disabled={busy} className="flex min-h-40 w-full flex-col items-center justify-center rounded-3xl border-2 border-dashed border-brand-indigo/30 bg-gradient-to-br from-brand-indigo/[.04] via-white to-brand-coral/[.05] p-4 text-center outline-none transition-colors hover:border-brand-indigo focus-visible:ring-2 focus-visible:ring-brand-indigo disabled:opacity-60"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-brand-indigo text-white"><Upload className="h-6 w-6" /></span><b className="mt-3">Upload Video</b><span className="mt-1 text-caption leading-4 text-brand-slate">MP4 or WebM · 60 sec · 100 MB</span></button>}</div> : <div className="mt-5 overflow-hidden rounded-3xl border border-brand-indigo/15 bg-brand-midnight"><video src={preview} controls playsInline preload="metadata" aria-label="GLIMPS video preview" className="mx-auto block aspect-[9/16] max-h-[45dvh] w-full max-w-[18rem] bg-brand-midnight object-contain" /><div className="flex flex-wrap items-center justify-between gap-2 bg-white p-3"><div className="min-w-0"><p className="truncate text-body-sm font-bold">{isRecorded ? "Recorded GLIMPS" : "Selected video"}</p><p className="text-caption text-brand-slate">{metadata?.durationSeconds}s · {metadata?.width} × {metadata?.height}</p></div><div className="flex gap-2"><button type="button" onClick={clear} disabled={busy} className="rounded-xl border border-brand-borderLight px-3 py-2 text-caption font-bold text-brand-indigo">{isRecorded ? "Retake" : "Replace"}</button><button type="button" onClick={clear} disabled={busy} aria-label="Remove selected video" className="rounded-xl px-3 py-2 text-caption font-bold text-brand-coral">Remove</button></div></div></div>}
