...
export default function GlimpsCreateComposer({ profile, organizations }: Props) {
  ...
  return <section className="rounded-3xl border border-brand-borderLight bg-white p-4 pb-[calc(6.5rem+env(safe-area-inset-bottom))] text-brand-midnight shadow-elevated sm:p-7 sm:pb-7">
    ...
    <div className="fixed inset-x-0 bottom-[calc(4.25rem+env(safe-area-inset-bottom))] z-40 border-t border-brand-borderLight bg-white/95 px-4 py-3 backdrop-blur sm:static sm:mt-6 sm:border-0 sm:bg-transparent sm:p-0"><button type="button" onClick={publish} disabled={!file || !metadata || busy} className="mx-auto flex min-h-12 w-full max-w-2xl justify-center gap-2 rounded-xl bg-brand-indigo px-5 py-3 font-bold text-white shadow-elevated disabled:opacity-60">{busy && <Loader2 className="h-4 w-4 animate-spin" />}{status === "preparing" ? "Preparing…" : status === "uploading" ? "Uploading video…" : status === "publishing" ? "Publishing GLIMPS…" : status === "posted" ? "GLIMPS posted." : "Publish GLIMPS"}</button></div>
  </section>;
}
