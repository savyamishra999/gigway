...
export default function GlimpsCreateComposer({ profile, organizations }: Props) {
  ...
  const selectFile = async (candidate?: File) => {
    ...
    if (!isValidGlimpsVideoMime(candidate.type)) return setError("GLIMPS supports MP4 and WebM video.");
    if (candidate.size > MAX_BYTES) return setError("This video is larger than 100 MB.");
    setStatus("preparing");
    try {
      const next = await readMetadata(candidate);
      if (next.durationSeconds > 60) throw Error("This video is longer than 60 seconds.");
