...
export async function safePost(post:SocialPost, viewerId?:string|null) {
  const db = socialDb()
  const [profileRes, orgRes, likeRes, commentRes, likedRes, savedRes, mediaRes] = await Promise.all([
  ...
  return { id:post.id, body:post.body, postType:post.post_type, visibility:post.visibility, createdAt:post.created_at, editedAt:post.edited_at, author, media:media.filter(Boolean), likeCount:likeRes.count||0, commentCount:commentRes.count||0, isLikedByMe:!!likedRes.data, isSavedByMe:!!savedRes.data }
}
