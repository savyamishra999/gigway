import { Metadata } from "next"
import { redirect } from "next/navigation"
import Link from "next/link"
import { createClient } from "@/lib/supabase/server"
import { FileText } from "lucide-react"
import AdminGigsClient from "@/components/admin/AdminGigsClient"

export const metadata: Metadata = { title: "Admin — Gigs — GigWay" }

const ADMIN_EMAILS = (process.env.ADMIN_EMAILS || "tellitorg1@gmail.com").split(",").map(e => e.trim())
const PAGE_SIZE = 20

type CatFilter = "all" | "design" | "development" | "writing" | "marketing"
interface SearchParams { cat?: string; page?: string; q?: string }

export default async function AdminGigsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user || !ADMIN_EMAILS.includes(user.email ?? "")) redirect("/")

  const params = await searchParams
  const cat    = (params.cat ?? "all") as CatFilter
  const page   = Math.max(1, parseInt(params.page ?? "1"))
  const q      = params.q?.trim() ?? ""
  const offset = (page - 1) * PAGE_SIZE

  let query = supabase
    .from("gigs")
    .select(
      `id, title, price, category, delivery_days, status, created_at,
       seller:freelancer_id(full_name, email)`,
      { count: "exact" }
    )

  if (q)          query = query.ilike("title", `%${q}%`)
  if (cat !== "all") query = query.ilike("category", `%${cat}%`)

  const { data: raw, count } = await query
    .order("created_at", { ascending: false })
    .range(offset, offset + PAGE_SIZE - 1)

  type GRow = {
    id: string; title: string; price: number | null; category: string | null
    delivery_days: number | null; status: string | null; created_at: string
    seller: { full_name: string | null; email: string | null } | { full_name: string | null; email: string | null }[] | null
  }

  const gigs = (raw as unknown as GRow[] ?? []).map(g => {
    const s = Array.isArray(g.seller) ? g.seller[0] : g.seller
    return { ...g, seller_name: s?.full_name ?? null, seller_email: s?.email ?? null }
  })

  const totalPages = Math.ceil((count ?? 0) / PAGE_SIZE)

  const CATS: { key: CatFilter; label: string }[] = [
    { key: "all",         label: "All" },
    { key: "design",      label: "Design" },
    { key: "development", label: "Dev" },
    { key: "writing",     label: "Writing" },
    { key: "marketing",   label: "Marketing" },
  ]

  function tabUrl(c: CatFilter) {
    const sp = new URLSearchParams()
    if (c !== "all") sp.set("cat", c)
    if (q) sp.set("q", q)
    const s = sp.toString()
    return `/admin/gigs${s ? `?${s}` : ""}`
  }

  function pageUrl(p: number) {
    const sp = new URLSearchParams()
    if (cat !== "all") sp.set("cat", cat)
    if (q) sp.set("q", q)
    if (p > 1) sp.set("page", String(p))
    const s = sp.toString()
    return `/admin/gigs${s ? `?${s}` : ""}`
  }

  return (
    <div className="min-h-screen bg-[#0A0A0F] py-10 px-4">
      <div className="max-w-6xl mx-auto space-y-6">

        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#4F46E5]/10 flex items-center justify-center">
            <FileText className="h-5 w-5 text-[#818CF8]" />
          </div>
          <div>
            <h1 className="text-2xl font-black text-white">Gigs</h1>
            <p className="text-[#6B7280] text-xs mt-0.5">{(count ?? 0).toLocaleString()} total</p>
          </div>
        </div>

        <form method="GET" action="/admin/gigs" className="flex gap-3">
          {cat !== "all" && <input type="hidden" name="cat" value={cat} />}
          <input name="q" defaultValue={q} placeholder="Search by title or seller..."
            className="flex-1 bg-[#12121A] border border-[#1E1E2E] text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#4F46E5] placeholder:text-[#475569]" />
          <button type="submit"
            className="px-5 py-2.5 bg-[#4F46E5] text-white text-sm font-semibold rounded-xl hover:bg-[#4338CA] transition-colors">
            Search
          </button>
          {q && (
            <Link href={tabUrl(cat)}
              className="px-4 py-2.5 border border-[#1E1E2E] text-[#6B7280] text-sm rounded-xl hover:text-white hover:border-[#334155] transition-colors">
              Clear
            </Link>
          )}
        </form>

        <div className="flex gap-2 flex-wrap">
          {CATS.map(c => (
            <Link key={c.key} href={tabUrl(c.key)}
              className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${
                cat === c.key
                  ? "bg-[#4F46E5] text-white"
                  : "bg-[#12121A] border border-[#1E1E2E] text-[#6B7280] hover:text-white hover:border-[#334155]"
              }`}>
              {c.label}
            </Link>
          ))}
        </div>

        <AdminGigsClient initial={gigs} />

        {totalPages > 1 && (
          <div className="flex items-center justify-between">
            <p className="text-[#6B7280] text-xs">Page {page} of {totalPages}</p>
            <div className="flex gap-2">
              {page > 1 && (
                <Link href={pageUrl(page - 1)}
                  className="px-4 py-2 border border-[#1E1E2E] text-[#6B7280] text-sm rounded-xl hover:text-white hover:border-[#334155] transition-colors">
                  Previous
                </Link>
              )}
              {page < totalPages && (
                <Link href={pageUrl(page + 1)}
                  className="px-4 py-2 bg-[#4F46E5] text-white text-sm font-semibold rounded-xl hover:bg-[#4338CA] transition-colors">
                  Next
                </Link>
              )}
            </div>
          </div>
        )}
      </div>
    </div>
  )
}
