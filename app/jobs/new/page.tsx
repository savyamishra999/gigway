...
export default async function NewJobPage({ searchParams }: { searchParams: Promise<{ organization?: string }> }) {
  ...
  const initialOrganizationId = organizations.some((organization: any) => organization.id === requested) ? requested : ""
  return (
    <div className="min-h-screen bg-[#0F172A] py-10">
      <div className="container mx-auto px-4 max-w-2xl">
        <JobForm userId={user.id} organizations={organizations} initialOrganizationId={initialOrganizationId} />
      </div>
    </div>
  )
}
