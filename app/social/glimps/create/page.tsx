...
export default async function CreateGlimpsPage() {
  ...
  if (!user) redirect(await loginForCurrent("/social/glimps/create"));
  const db = socialDb();
  ...
  return <main className="bg-brand-ivory px-4 py-6 pb-[calc(6.5rem+env(safe-area-inset-bottom))] sm:min-h-screen sm:py-10"><div className="mx-auto max-w-2xl"><GlimpsCreateComposer profile={{ name: profile.full_name || "My profile", avatar: profile.avatar_url }} organizations={(organizations || []).map((organization) => ({ id: organization.id, name: organization.name, avatar: organization.logo_url }))} /></div></main>;
}
