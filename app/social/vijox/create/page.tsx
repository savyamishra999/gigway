...
export default async function CreateJoxPage() {
  ...
  return <main className="min-h-screen bg-brand-ivory px-4 py-6 pb-24 sm:py-10"><div className="mx-auto max-w-2xl"><CreatePostComposer mode="jox" profile={{ name: profile.full_name || "My profile", avatar: profile.avatar_url }} organizations={(organizations || []).map(organization => ({ id: organization.id, name: organization.name, avatar: organization.logo_url }))} /></div></main>;
}
