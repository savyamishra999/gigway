...
import JoxCreateComposer from "@/components/social/JoxCreateComposer";

export default async function CreateJoxPage() {
  const session = await createClient(), { data: { user } } = await session.auth.getUser();
  if (!user) redirect(await loginForCurrent("/social/vijox/create"));
  const db = socialDb();
  ...
  return <main className="min-h-screen bg-brand-ivory px-4 py-6 pb-24 sm:py-10"><div className="mx-auto max-w-2xl"><JoxCreateComposer profile={{ name: profile.full_name || "My profile", avatar: profile.avatar_url }} organizations={(organizations || []).map(organization => ({ id: organization.id, name: organization.name, avatar: organization.logo_url }))} /></div></main>;
}
