...
import { canManagePost, GLIMPS_MAX_FILE_SIZE_BYTES, POST_MEDIA_BUCKET, requireSocialUser, socialDb, validMediaMetadata } from "@/lib/social/server";
...
export async function POST(req:NextRequest,{params}:{params:Promise<{id:string}>}) {
  ...
  const {id}=await params, body=await req.json().catch(()=>({})), size=body.fileSize??body.size;
  ...
  if(domain==="glimps"&&(!isValidGlimpsVideoMime(body.mimeType)))return NextResponse.json({error:"GLIMPS supports MP4 and WebM video."},{status:400});
  if(domain==="glimps"&&(typeof size!=="number"||size>GLIMPS_MAX_FILE_SIZE_BYTES))return NextResponse.json({error:"This video is larger than 100 MB."},{status:400});
  const rule=validMediaMetadata(body.fileName,body.mimeType,size);
  if(!rule)return NextResponse.json({error:domain==="glimps"?"GLIMPS supports MP4 and WebM video.":"Only JPG, PNG, WebP (10 MB), MP4/WebM (100 MB), PDF (20 MB), or VIJOX audio (10 MB) files are allowed."},{status:400});
