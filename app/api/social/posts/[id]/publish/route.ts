...
export async function POST(_req:NextRequest,{params}:{params:Promise<{id:string}>}) {
  ...
  if(isGlimps(post.content_format)&&!validGlimpsMedia(media||[]))return NextResponse.json({error:"A GLIMPS requires one MP4 video of 60 seconds or less and no more than 100 MB."},{status:400});
