...
export async function GET(req:NextRequest){
  ...
  if(feed==="discover"){
    ...
    const {data,error}=await q;if(error)return NextResponse.json({error:"We could not load the feed."},{status:503});const accessible=[] as any[];for(const post of data||[]){if(await canViewPost(post,viewer?.id))accessible.push(post)}const page=accessible.slice(0,PAGE_SIZE);return NextResponse.json({items:await Promise.all(page.map(p=>safePost(p,viewer?.id))),nextCursor:accessible.length>PAGE_SIZE?`${page.at(-1).created_at}|${page.at(-1).id}`:null})
  }
