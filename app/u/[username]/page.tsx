...
export default async function PublicIdentity({ params }: { params: Promise<{ username: string }> }) {
  ...
  const [{ count: followerCount }, { data: membership }] = await Promise.all([socialDb().from("organization_follows").select("organization_id", { count: "exact", head: true }).eq("organization_id", organization.id), viewer ? socialDb().from("organization_members").select("member_role").eq("organization_id", organization.id).eq("profile_id", viewer.id).eq("status", "active").maybeSingle() : Promise.resolve({ data: null })])
